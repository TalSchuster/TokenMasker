...
@Model.register("esim_wmask")
class ESIM_wmask(Model):
    """
    ...
    def forward(self,  # type: ignore
                ...
                premise_delete: torch.LongTensor = None
        ...
        premise_delete_sents = (first_inds != -1).long()
        teacher_available = premise_delete_sents.nonzero()
        if len(teacher_available) > 0:
            premise_delete_mask = (premise_delete != -1).float() # Assumes sentences without premise delete are all -1
            premise_delete = premise_delete * premise_delete_mask
            premise_delete = premise_delete * premise_mask
            assert (embedded_premise.shape[:2] == premise_delete.shape)
            # Remove the deleted ones
            amortize_embedded_premise = embedded_premise * (1 - premise_delete[:,:,None])

            # Replace the deleted ones with oov emb
            oov_emb = self._text_field_embedder._modules['token_embedder_tokens'].weight[self.vocab.get_token_index(self.vocab._oov_token),:]
            amortize_embedded_premise += premise_delete[:,:,None] * oov_emb

            embedded_premise = amortize_embedded_premise
